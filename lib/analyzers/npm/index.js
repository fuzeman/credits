...
function getNpmCredits(packagePath, credits) {
    ...
    (
      ...
      fs.lstatSync(directoryPath).isSymbolicLink()
		...
		if (author) {
			credits = creditUtil.addCreditToCredits(credits, author, packageJson['name'] || name);
		}

		if (maintainers) {
			maintainers.forEach(maintainer => {
				credits = creditUtil.addCreditToCredits(credits, maintainer, packageJson['name'] || name);
			});
		}
	}
